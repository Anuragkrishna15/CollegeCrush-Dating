...
DROP POLICY IF EXISTS "Chat media is accessible to conversation participants" ON storage.objects;
...
-- Note: storage.objects is a system table managed by Supabase
-- We cannot modify its schema directly
-- File size validation is handled on the client side
...
BEGIN
    ...
    -- Note: File size validation is handled on the client side
    -- Supabase storage may not provide file_size in the trigger context
